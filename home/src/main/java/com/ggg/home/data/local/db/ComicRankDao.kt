...
abstract class ComicRankDao {
    @Transaction
    @Query("SELECT comic.* FROM ComicRankModel comic join CategoryOfComicModel cate on comic.id = cate.comicId where 1 = 1 and (comic.latestChapter IS NOT NULL and comic.latestChapter <> '') and comic.type = :type GROUP BY comic.id ORDER BY lastModified ASC limit :limit offset :offset")
